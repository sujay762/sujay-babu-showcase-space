...
import { about, education, loadDynamicData } from '@/lib/data';
      ...
      <div className="py-10 relative">
        ...
        <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {about.experience.map((exp, index) => (
                <div 
                  key={index} 
                  className={cn(
                    "glass-panel p-5 rounded-lg relative hover-lift transform transition-all duration-700",
          ...
          <section className="mb-16">
            ...
            <div 
              className={cn(
                "grid grid-cols-2 md:grid-cols-4 gap-4 transform transition-all duration-1000",
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              )}
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {education.map((edu, index) => (
                ...
                >
                  ...
                  <div className="text-sm text-emerald-500 font-medium">{edu.period}</div>
                  <div className="text-xl font-bold mt-1">{edu.institution}</div>
                  <div className="text-gray-600 mb-2">{edu.degree}</div>
                  <div className="text-sm text-gray-500 mb-2">{edu.location}</div>
                  
                  {edu.gpa && (
                    <div className="text-sm font-medium">
                      GPA: <span className="text-emerald-500">{edu.gpa}</span>
                    </div>
                  )}
                </div>
              ))}
